    ...
    while True:
        ...
        # mod(Forgeモード)の動作
            ...
            while True:
                ...
                if not os.file.exists(input_jar_file_install):
                    print("ファイルが存在しません")
                    continue
    ...
    for i in range(server_count):
        ...
        if server.local_jar:
            ...
            if server.local_jar_mode == 1:
                ...
                local_jar.start_jar_file = local_jar.install_jar_file.replace('-installer', '')
                shutil.copy(server.local_jar_file, const.minecraft_dir+"/"+local_jar.install_jar_file)
                shutil.copy(server.local_jar_file, const.minecraft_dir+"/"+local_jar.start_jar_file)
            else:
                ...
                # ダウンロード時の例外処理
                    ...
                    sys.exit(1)
        # eula.txtファイル上書き
        etc_server.file_write(const.minecraft_dir+"/eula.txt", "#By changing the setting below to TRUE you are indicating your agreement to our EULA (https://account.mojang.com/documents/minecraft_eula).\n#"+detailed_day[1]+" "+detailed_day[0]+" "+dt_now.strftime('%d')+" "+dt_now.strftime('%H:%M:%S')+" "+str(dt_now.tzinfo)+" "+dt_now.strftime('%Y')+"\neula="+str(choice))
